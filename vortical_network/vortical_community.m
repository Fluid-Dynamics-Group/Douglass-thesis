function [Ci,xc,yc,Gamma_c,A] = vortical_community(xfil,yfil,Gamma_v,flag, gamma, fluid)
                                                 %(xfil,yfil,Gamma_v,   1, gamma, 1)
...
A = adjacency_mat(xfil,yfil,Gamma_v,1,'alg',fluid); % sam added ,fluid

%% Identify communities in the vortical network
% (Split A into +ve & -ve vortical elements & then find communities)
if flag == 1
    [Ci,numcom] = find_vortical_communities(Gamma_v,A,gamma); %0.75
else
    [~,Ci] = find_communities(gamma,1:length(Gamma_v),A);
...
%% ***********************Functions**************************************
function [A] = adjacency_mat(x,y,Gamma,alpha,type, fluid)
%%
...
fprintf('Calculating adjacency matrix of %i nodes...\n',length(x));
A = NaN(pts,pts);
for i = 1:pts   % can use parfor if needed
    ...
    elseif strcmp(type,'geom') == 1
        ...
        error('Enter valid network definition "type"')
    end
end
...
Ci_full(id_n) = Ci_n;
if (~isempty(id_p))
Ci_full(id_n) = Ci_full(id_n) + numel(unique(Ci_p));
end
end
